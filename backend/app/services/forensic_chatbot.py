...
Tool:"""
    ...
    def _keyword_classify(self, message: str) -> Optional[str]:
        ...
        # Knowledge Graph specific queries (anomalies in graph, GDS, paths, transactions, calls, towers)
        if any(k in msg for k in ["pagerank", "betweenness", "centrality", "community id", "gds", "graph data science", "graph anomal", "knowledge graph"]):
            return "NEO4J_GRAPH"
...
Cypher:"""
        ...
        if any(k in p_low for k in ["transfer", "bank", "amount", "money", "transaction"]) and not any(k in p_low for k in ["path", "shortest", "hop"]):
            ...
            if name_match:
                ...
                if name_term and len(name_term) > 2 and name_term.lower() not in ["the", "all", "account", "bank"]:
                    return f"MATCH (p:Person)-[:OWNS_ACCOUNT]->(b1:BankAccount)-[r:TRANSFERS_MONEY|TRANSACTED_WITH]-(b2:BankAccount)\nWHERE (p.case_id IN $case_ids OR ANY(cid IN $case_ids WHERE cid IN coalesce(p.case_ids, []))) AND toLower(p.name) CONTAINS toLower('{name_term}')\nRETURN p.name AS person_name, b1.account_number AS sender_account, coalesce(r.amount_inr, r.amount, 0) AS amount, b2.account_number AS receiver_account, coalesce(b2.holder, b2.name, 'Recipient') AS receiver_holder\nORDER BY amount DESC LIMIT 25"
            return (
                ...
                "RETURN b1.account_number AS sender_account, coalesce(b1.holder, b1.name, 'Sender') AS sender_holder,\n"
                "       coalesce(r.amount_inr, r.amount, 0) AS amount, b2.account_number AS recipient_account, coalesce(b2.holder, b2.name, 'Recipient') AS recipient_holder\n"
                "ORDER BY amount DESC LIMIT 25"
            )
            ...
            return (
                ...
                "RETURN p1.number AS caller, coalesce(r.duration_seconds, r.duration, 0) AS duration, p2.number AS receiver\n"
                "ORDER BY duration DESC LIMIT 25"
            )
        elif any(k in p_low for k in ["tower", "cell", "bts"]):
            return (
                "MATCH (p:Phone)-[r:LOCATED_AT|PINGED_TOWER]-(t:CellTower)\n"
                "WHERE (p.case_id IN $case_ids OR ANY(cid IN $case_ids WHERE cid IN coalesce(p.case_ids, [])))\n"
                "RETURN p.number AS phone, t.tower_id AS tower_id, coalesce(t.address, t.location, t.tower_id) AS location, coalesce(r.pings_count, 1) AS pings\n"
                "LIMIT 25"
            )
        elif any(k in p_low for k in ["anomal", "threat", "suspicious"]) and any(k in p_low for k in ["graph", "node", "edge", "knowledge"]):
            name_match = re.search(r'(?:for|of|by|about)\s+([a-zA-Z\s]+)', prompt, re.IGNORECASE)
            name_term = name_match.group(1).strip() if name_match else ""
            if name_term and len(name_term) > 2 and name_term.lower() not in ["the", "all", "case"]:
                return f"MATCH (p:Person)-[r:HAS_ANOMALY]->(a:Anomaly)\nWHERE (p.case_id IN $case_ids OR ANY(cid IN $case_ids WHERE cid IN coalesce(p.case_ids, []))) AND toLower(p.name) CONTAINS toLower('{name_term}')\nRETURN p.name AS person, a.title AS anomaly_title, a.severity AS severity, a.domain AS domain, coalesce(a.score, 0.0) AS score, a.whatHappened AS what_happened, a.whyUnusual AS why_unusual\nORDER BY score DESC LIMIT 25"
            return (
        ...
        elif any(k in p_low for k in ["pagerank", "betweenness", "centrality", "community", "gds", "algorithm"]):
            return (
                "MATCH (p:Person)\n"
                "WHERE (p.case_id IN $case_ids OR ANY(cid IN $case_ids WHERE cid IN coalesce(p.case_ids, [])))\n"
                "  AND (p.pagerank IS NOT NULL OR p.communityId IS NOT NULL)\n"
                "RETURN p.name AS person, p.pagerank AS pagerank, p.betweenness AS betweenness, p.communityId AS community_id\n"
                "ORDER BY p.pagerank DESC LIMIT 25"
            )
    ...
    def _fallback_cypher(self, prompt: str, case_ids: List[str]) -> str:
        ...
        elif any(k in p_low for k in ["anomal", "threat", "suspicious"]):
            return (
                "MATCH (p:Person)-[r:HAS_ANOMALY]->(a:Anomaly)\n"
                "WHERE (p.case_id IN $case_ids OR ANY(cid IN $case_ids WHERE cid IN coalesce(p.case_ids, [])))\n"
                "RETURN p.name AS person, a.title AS anomaly_title, a.severity AS severity, a.domain AS domain, coalesce(a.score, 0.0) AS score, a.whatHappened AS what_happened, a.whyUnusual AS why_unusual\n"
...
Forensic Response:"""
    ...
    def process_chat_message(
        ...
        active_case = target_cids[0] if target_cids else (case_id or "INV-2026-BLACK-CIRCUIT")

        # ── STAIR STRUCTURE-AWARE EVIDENCE-FIRST ENGINE ──────────────
        if use_stair and getattr(stair_service, "enabled", True):
            try:
                stair_res = stair_service.investigate(
                    query=message,
                    case_id=active_case,
                    history=history
                )
                logger.info(f"[Chatbot] STAIR execution successful. Selected leaves: {stair_res.selected_leaf_ids}")
                return {
                    "reply": stair_res.reply,
                    "tool_used": stair_res.tool_used,
                    "generated_cypher": stair_res.generated_cypher,
                    "sql_query": stair_res.sql_query,
                    "records_count": stair_res.records_count,
                    "records": stair_res.records,
                    "resolved_entities": stair_res.resolved_entities,
                    "anomalies": stair_res.anomalies,
                    "alerts": stair_res.alerts,
                    "suggested_followups": stair_res.suggested_followups,
                    "model_used": stair_res.model_used,
                    "stair_leaf_ids": stair_res.selected_leaf_ids,
                    "claim_mappings": [c.model_dump() for c in stair_res.claim_mappings],
                    "abstention_triggered": stair_res.abstention_triggered,
                    "latency_breakdown": stair_res.latency_breakdown_ms
                }
            except Exception as e:
                logger.warning(f"[Chatbot] STAIR execution encountered error, falling back to legacy: {e}", exc_info=True)

        conversation_context = self._build_conversation_context(history)

        # ── Legacy Step 1: Classify Tool ───────────────────────────
